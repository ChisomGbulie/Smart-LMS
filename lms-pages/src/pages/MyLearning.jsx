...
export default function MyLearning() {
  ...
  // Helper functions to get real user data from localStorage
  ...
  const getLastAccessed = (courseId) => {
    const lastAccessed = localStorage.getItem(`course_${courseId}_lastAccessed`);
    return lastAccessed || new Date().toISOString();
  };
  ...
  const loadEnrolledCourses = async () => {
    ...
    try {
      ...
      // Get REAL enrolled courses from localStorage (NOT from initialProgress placeholder)
      const enrolledCourseIds = getEnrolledCourseIds();
      
      // Filter courses that user is actually enrolled in
      const enrolled = courses
        ...
        .map(course => {
          ...
          return {
            ...
            nextModule: course.syllabus?.[Math.floor(course.syllabus.length * (progress / 100))] || course.syllabus?.[0]
          };
        });

      setEnrolledCourses(enrolled);

      // Calculate REAL statistics based on actual enrolled courses
      ...
      const totalHours = enrolled.reduce((sum, c) => {
      ...
      const completedHours = enrolled.reduce((sum, c) => {
        ...
        if (c.completed) return sum + validHours;
        return sum + (validHours * (c.progress / 100));
      }, 0);
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-8">
          ...
          {/* Stats Cards - Only show if user has enrolled courses */}
          ...
          {/* Filters and Search - Only show if user has enrolled courses */}
          {stats.totalCourses > 0 && (
        ...
        {stats.totalCourses === 0 ? (
          ...
          <div className="text-center py-16 bg-white rounded-xl shadow-sm border border-gray-100">
            ...
            <p className="text-gray-500 mb-6 max-w-md mx-auto">
              You haven't enrolled in any courses yet. Browse our course catalog and find the perfect course for your career goals.
            </p>
            <button
              onClick={() => navigate('/courses')}
        ...
        {/* Achievement Section - Only if user has completed courses */}
        ...
        {/* Recommended Next Steps - Only if user has in-progress courses */}
        {filteredCourses.length > 0 && stats.inProgressCourses > 0 && (
          <div className="mt-8 bg-blue-50 rounded-xl p-6 border border-blue-100">
            <div className="flex items-center gap-2 mb-3">
              <Target className="h-5 w-5 text-blue-600" />
              <h3 className="font-semibold text-gray-900">Continue Your Learning</h3>
            </div>
